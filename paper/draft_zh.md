...
![图 7. 机筒网消融（Constrained-SAC，种子均值±std）。(a) 24 小时成本（标注终端缺口）；(b) 违规小时。移除可行性实现（红）破坏可行性并使成本翻倍；移除软边界（绿）恢复近最优成本且零违规；归零喘振/终端惩罚权重无影响（约束 SAC 用统一约束成本）。](fig_ablation.png)
...
![图 8. 枝状基准网——各方法标称 24 小时成本（均值±std）。可行性感知的 DRL 智能体保持在带内；SAC 与 GA 最优成本相当。](../models/exports/paper_branched/fig_cost_nominal.png)

![图 9. 枝状基准网——扰动需求/电价下的鲁棒性（成本均值±std 与违规小时）。](../models/exports/paper_branched/fig_robustness.png)
...
![图 10. GasLib-40 衍生网络——各方法标称 24 小时成本（均值±std）。](../models/exports/paper_gaslib/fig_cost_nominal.png)
...
### 6.10 汇总：成本-延迟权衡与跨网络一致性

两张汇总图浓缩全文。图 11 在机筒网上画**成本对部署延迟**：基于模型的优化器（DP/MPC/GA）成本低但延迟高（1–11 s），从头 DRL 快（约 5 ms）但有成本溢价，而 **Distilled-MPC 占据理想的左下角**——成本最低*且*毫秒延迟。图 12 把各方法成本按每网络的 DP 最优归一化、跨三网络比较：GA、MPC、Distilled-MPC 处处≈1×，而从头 SAC/Constrained-SAC 一致带约 2–3× 溢价（唯一例外是 SAC 在枝状网，因其粗 3-D 网格 DP 参考本身偏松）。两图合起来正是本文论点：*蒸馏在实时延迟下给出近最优成本、跨网络一致；从头 RL 快且可行但成本次优。*

![图 11. 机筒网成本对部署延迟（延迟对数轴；绿=可行，红=不可行；★=Distilled-MPC）。左下更优——又便宜又快。](fig_cost_latency.png)

![图 12. 各关键方法在机筒/枝状/GasLib-40 三网络上按每网络 DP 最优归一化的标称成本（×1 虚线）。Distilled-MPC 处处≈最优；从头 SAC/Constrained-SAC 带溢价。](fig_summary_networks.png)
